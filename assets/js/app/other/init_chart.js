...
// Custom colors specific, includes the new "grays".
var newColors = {
...
var overrides = window.Theme && window.Theme.colors ? window.Theme.colors : {};
...
if (Object.keys(overrides).length !== 0 && overrides.constructor === Object) {
    for (var colorName in overrides) {
        if (overrides.hasOwnProperty(colorName)) {
            if (!/^#([A-Fa-f0-9]{3}$)|([A-Fa-f0-9]{6}$)/.test(overrides[colorName])) throw new Error('Please provide a hexadecimal color value if you are trying to override the colors.');
...
if (typeof Chart === 'undefined') {
    throw new Error('requires the Chart.js library in order to work properly.');
}

window.Theme = window.Theme ? window.Theme : {};
window.Theme.colors = colors$1;
